...
vi.mock('@/auth-provider', () => ({
    useAuth: vi.fn(() => ({
        user: { uid: 'sup-1', email: 'sup@example.com' },
        loading: false
    }))
}));
...
describe('SupervisorDashboard', () => {
    beforeEach(() => {
        ...
        mockUseDoc.mockImplementation((ref) => {
            ...
            if (path === 'users/sup-1') {
                return { data: { role: 'supervisor', name: 'Test Supervisor' }, isLoading: false };
            }
        ...
        mockUseCollection.mockImplementation((ref) => {
            ...
            // Mock production logs for ProductionLog component
            // Mock production logs for ProductionLog component
            if (path === 'production') {
                return {
                    data: [
            ...
            // Mock operators for dashboard stats/tables
            if (path === 'users') {
                return {
                    data: [
            ...
            if (path === 'styles') {
                return {
                    data: [
                        { id: 'style-1', name: 'Style A', status: 'active', operations: [{ id: 'op-1', smv: 1 }] }
                    ],
                    isLoading: false
                };
            }
    ...
    it('renders the dashboard stats cards', async () => {
        ...
        await expect(screen.findByText('Daily Production', {}, { timeout: 3000 })).resolves.toBeDefined();
        await expect(screen.findByText('Floor Efficiency')).resolves.toBeDefined();
    });

    it('displays the overview tab content', () => {
        render(<SupervisorDashboard />);
