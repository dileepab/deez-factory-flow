...
import type { Operator, Configuration } from '@/lib/types';
...
export function SalarySlip({ operator, totalEarnedMinutes, daysAbsent, config }: SalarySlipProps) {
  ...
  // Calculate salary using the centralized function and DB-driven config
  const { finalSalary, basePay, attendanceBonus } = calculateMonthlySalary(
    totalEarnedMinutes,
    daysAbsent,
    ...
    <Card id="earnings" className="w-full max-w-md mx-auto">
      ...
      <CardContent className="space-y-4">
        <div className="flex justify-between items-center">
          <span className="text-muted-foreground">Base Pay (from minutes)</span>
